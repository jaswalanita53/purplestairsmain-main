...
class ArchivedSearch extends Component
{
    ...
    public function mount()
    {
        ...
        $this->search = Search::where('slug', $this->slug)->with('newMatch')->with('searchUser')->withTrashed()->first();
        $search = $this->search;
    ...
    public function render()
    {
        ...
        $company_users = [];
        $search = Search::where('slug', $this->slug)->with('newMatch')->with('searchUser')->withTrashed()->first();


        $new_candidates_ids =$search->newMatch->pluck('match_uid')->toArray();
        $new_candidates_count = count($new_candidates_ids);
        $past_search_match_users= $past_search_match_users = json_decode($search->synced_users, true);
        // \DB::enableQueryLog();
        if(!empty($search->id)){
        $this->my_favorites = Favorite::where('company_id', $this->company)->where('search_id',$search->id)->pluck('candidate_id')->toArray();
        }
        $search_uids =$search->searchUser->pluck('user_id')->toArray();
        $search_count = count($search->searchUser);
        ...
        $zipcode_val = ($this->sort_order == 'DESC') ? 0 : 9999999;
        ...
        $candidates = $candidates->whereIn('id',$search_uids);
        ...
        $this->all_zipcodes = ZipCode::pluck('zip_code')->toArray();
        return view('livewire.archived-search', compact('candidates', 'search', 'favorites_count', 'requested_count', 'unmasked_count', 'past_search_match_users', 'new_candidates_ids', 'filteredUserCount', 'company_users'));
    }
