...
<script>
  ...
  // $(function () {
    ...
    $('#employersTable').DataTable({
      "serverSide": true,
      "ajax":{
        url :"{{ route('admin.invited.users.data') }}",
        type: "POST",
        data: {
          '_token': '{{ csrf_token() }}',
      ...
      columns: [
        ...
        { data: "email" },
        { data: "date" },
        { data: "invited_by", orderable: false, render: function(data, type, row, meta) {
            return '<a class="" href="{{ url('/admin/employers/view/') }}/'+row.invited_by+'">'+row.invited_by_email+'</a>';
        ...
        { data: null, orderable: false, render: function(data, type, row, meta) {
          let _actions = "";
          if(row.status == 3) {
            _actions += '<a class="btn btn-success btn-sm" href="{{ url('admin/invited/active') }}/'+row.id+'">Approve</a>';
          }else{
            return 'N/A';
;          }
          return _actions;
