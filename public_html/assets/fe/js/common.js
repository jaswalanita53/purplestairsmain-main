jQuery(document).ready(function ($) {
  ...
  // })
    ...
    // 86a2p14u2
    $(".switch_1:not(.checkPrice)").each(function (index, el) {
    $(el).change(function (e) {
      /*console.log('el',el,$(el).is(":checked"),$(el).prop("checked"));
      if ($(el).is(":checked")) {
      ...
      // choose plan page logic
      /*
      * plan 1 : monthly = 299, yearly = 2999
      * plan 2 : monthly = 749, yearly = 8500
      */
      // console.log(e.target.getAttribute('ref'));
      if ($(el).attr('ref') !== undefined) {
        var ref_el = $(el).attr('ref');
        var _amount = null;
        var _label = null;
        if ($(el).prop("checked")) {
          _label = "Month";
          if (ref_el == "price-info-first") { _amount = 299; } 
          else { _amount = 749; }

        } else {
          _label = "Year";
          if (ref_el == "price-info-first") { _amount = 2999; } 
          else { _amount = 8500; }
        }
...
$("body").delegate(".zip_code_field", "input", function (e) {
...
$("body").delegate(".num_of_emp", "input", function (e) {
...
$(".blue_btn_plan").addClass("shake-blue");
  setTimeout(function () {
    $(".blue_btn_plan").removeClass("shake-blue");
    currentTime = new Date();
    milliseconds = currentTime.getTime(); // Get the milliseconds from the date
  }, 4000);
$(".publish_profile_btn").addClass("shake-blue-publish");
  setTimeout(function () {
    $(".publish_profile_btn").removeClass("shake-blue-publish");
    currentTime = new Date();
    milliseconds = currentTime.getTime(); // Get the milliseconds from the date
  }, 4000);
})
    ...
    if (title == "" || message == "") {
      if(title==""){
        ...
        $(this).parents('form').find('.form-group:first').append(
          '<div class="text-danger text-start" style=""> This field is required.</div>'
        );
      }
      if(message==""){
...
//   // Get the previous URL from the document.referrer property
//   var previousURL = document.referrer;
// console.log(previousURL);
//   history.pushState({ url: previousURL }, null, previousURL);
//   window.onpopstate = function(event) {
//     if (event.state && event.state.url) {
//       // Navigate to the custom URL from the custom state
//       window.location.href = event.state.url;
//     } else {
//       // If the custom state is not available, use the default back button behavior
//       window.history.back();
//     }
//   };
// });
...
$("body").delegate(".input-daterange input:first", "change", function(e) {
var inputDateStr = $(this).val();
    ...
    if (inputDate > currentDate) {
      ...
      $(this).parents('.gl-frm-outr').find('.form-group:last').prepend('<div class="error" style="margin-top: -10px;"><small class="text-danger">First Month can not be a future Month. </small></div>');
...
$("body").delegate(".input-daterange input:eq(1)", "change", function(e) {
...
var inputDateStr = $(this).val();
    ...
    if (inputDate > currentDate) {
      ...
      $(this).parents('.gl-frm-outr').find('.error').remove();
      $(this).parents('.gl-frm-outr').find('.form-group:last').prepend('<div class="error" style="margin-top: -10px;"><small class="text-danger">End Month can not be a future Month. </small></div>');
  ...
  $("body").delegate("input[type='password']", "keypress paste", function(e) {
  ...
  $("body").delegate(".password-field", "keypress", function(e) {
    if (e.which === 32) { // Check if the key pressed is the spacebar
              e.preventDefault();
    }
  });
  $("body").delegate(".password-field", "keyup", function(e) {
  ...
  $('select').on('select2:open', function (e) {
    $('.dd-cut').remove();
        $('.select2-dropdown').prepend('<i class="fa fa-times dd-cut"></i>');
})

    $("body").delegate(".dd-cut", "click", function(e) {
